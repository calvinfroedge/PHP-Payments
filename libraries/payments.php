...
class Payments
{
	...
	*/
	...
	public function __construct($config = null)
	{
		...
		//Load some lang files
		$this->lang = new stdClass();
		$this->lang->response_messages = $this->_load('lang', 'response_messages');
		$this->lang->response_details = $this->_load('lang', 'response_details');
	...
	*/
	protected function _load($type, $file)
	{
		...
		switch($type)
		{
			...
			case $type == 'library':
				$ob = true;
				$path = $base_dir.'/libraries/'.$file.'.php';
				break;

			case $type == 'lang':
				$ob = false;
				$language = (empty($this->config->main['language'])) ? 'english' : $this->config->main['language'];
				$path = $base_dir.'/language/'.$language.'/'.$file.'_lang.php';
				break;
	...
	{
		$supported_methods = $this->config->main['supported_methods'];
	...
	{	
		...
		{
			$this->config->module = $this->_load('config', 'payments/'.$payment_module);
	...
	private function _check_params($payment_module, $params)
	{
		...
		$req = $this->config->module['required_params'];
		...
		{	
			...
			{
				$missing[] = $this->lang->response_details['response_detail_missing_'.$v];
			}
		}
		...
		if(array_key_exists('cc_exp', $params))
		{
			...
			if(strlen($exp_date) != 6 OR !is_numeric($exp_date) OR $m1 > 1)
			{
				return $this->return_response(
					...
					'invalid_input', 
					'local_response', 
					$this->lang->response_message['response_detail_invalid_date_format']
				);
			}
		}
		...
		if(array_key_exists('billing_period', $params))
		{
			...
			if(!in_array($params['billing_period'], $accepted_billing_period))
			{
				return $this->return_response(
					...
					'invalid_input', 
					'local_response', 
					$this->lang->response_message['response_detail_invalid_billing_period']
				);			
			}
		}
	...
	private function _return_local_response($status, $response, $response_details = null)
	{
		...
		if(is_null($response_details))
		{
			...
			(
				...
				'response_message' 	=>	$this->lang->response_messages['response_message_'.$response]
			);			
		}
		else
		{
	...
	*/	
	...
	private function _connection_is_secure()
	{
		...
		if($this->config->main['force_secure_connection'] === FALSE) 
		{
