...
{	
	...
	*/		
	public function __construct($payments)
	{
		...
		$this->_api_endpoint = $this->payments->config->module['api_endpoint_'.$this->payments->mode];	
		$this->_fps_endpoint = $this->payments->config->module['fps_endpoint_'.$this->payments->mode];
		$this->_fps_version = $this->payments->config->module['fps_version'];
		$this->_fps_endpoint_parsed = parse_url($this->_fps_endpoint);		
		$this->_api_settings = array(
			'immediateReturn'	=> $this->payments->config->module['immediate_return'],
			'collectShippingAddress'	=> $this->payments->config->module['collect_shipping_address'],
			'signatureVersion' => (isset($payments->gateway_credentials)) ? $payments->gateway_credentials['api_signature_version'] : $this->payments->config->module['api_signature_version'],
			'signatureMethod'	=> (isset($payments->gateway_credentials)) ? $payments->gateway_credentials['api_signature_method'] : (isset($payments->gateway_credentials)) ? $payments->gateway_credentials['api_signature_method'] : $this->payments->config->module['api_signature_method'],	
			'accessKey' => (isset($payments->gateway_credentials)) ? $payments->gateway_credentials['api_access_key'] : $this->payments->config->module['api_access_key'],
			'isDonationWidget' => $this->payments->config->module['donation_widget'],
			'amazonPaymentsAccountId' => (isset($payments->gateway_credentials)) ? $payments->gateway_credentials['api_account_id'] : $this->payments->config->module['api_account_id'],
			'ipnUrl' => $this->payments->config->module['ipn_url'],
			'returnUrl' => $this->payments->config->module['return_url'],
			'cobrandingStyle' => 'logo',
			'abandonUrl' => $this->payments->config->module['abandon_url'],
		);
		$this->_secret_key = $this->payments->config->module['api_access_secret_key'];
		
		$this->_algorithm = $this->_api_settings['signatureMethod'];
		
		$custom_button = $this->payments->config->module['custom_button'];
		...
		{	
			$choices = explode('.', $this->payments->config->module['button_choice']);
			$style = $choices[0];
			$color = $choices[1];
			$size = $choices[2];
			
			$button = $this->payments->config->module['button_choices'];
