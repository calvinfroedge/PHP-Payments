...
class PayPal_PaymentsStandard
{
	...
	*/	
	private function _build_form()
	{
		...
		$this->_request['business'] = $this->payments->config->module['payment_email'];
		
		$ipn = $this->payments->config->module['ipn_url'];
	...
	*/		
	...
	{
		$map = $this->payments->config->module['payment_to_gateway_key_map'];
		$return = array();
	...
	*/	
	public function paypal_paymentsstandard_recurring_payment_button($params)
	{
		...
		$this->_request['src'] = ($this->payments->config->module['payments_never_expire'] === TRUE) ? 1 : 0;
		
		$this->_request['src'] = ($this->payments->config->module['retry_on_failure'] === TRUE) ? 1 : 0;
		
		$this->_request['no_note'] = ($this->payments->config->module['hide_note'] === TRUE) ? 1 : 0;
		
		$this->_request['modify'] = $this->payments->config->module['modify'];
		
		if($this->payments->config->module['user_manage']) $this->_request['usr_manage'] = 1;
