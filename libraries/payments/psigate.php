	...
	*/		
	...
	protected function _set_messenger_settings()
	{
		...
		$this->_api_settings = array(
			'cid'			=> (isset($payments->gateway_credentials)) ? $payments->gateway_credentials['api_cid'] : $this->payments->config->module['api_cid'],
			'store_id'		=> (isset($payments->gateway_credentials)) ? $payments->gateway_credentials['api_username'] : $this->payments->config->module['api_username'],
			'pass_phrase'	=> (isset($payments->gateway_credentials)) ? $payments->gateway_credentials['api_password'] : $this->payments->config->module['api_password'],
			'xml_version'	=> '1.0',
			'encoding'		=> 'utf-8',
			'xml_schema'	=> '',
		);		
	...
	{
		$this->_api_endpoint = $this->payments->config->module['api_recurring_endpoint'.'_'.$this->payments->mode];	
		
		$this->_api_settings = array(
			'cid'			=> $this->payments->config->module['api_cid'],
			'store_id'		=> $this->payments->config->module['api_username'],
			'pass_phrase'	=> $this->payments->config->module['api_recurring_password'],
			'xml_version'	=> '1.0',
			'encoding'		=> 'utf-8',
			'xml_schema'	=> '',
		);		
